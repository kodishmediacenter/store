...
AddonID = 'plugin.video.clientiptv'
...
addon_data_dir = xbmc.translatePath("special://home/addons/plugin.video.clientiptv/data").decode("utf-8")
...
def cs_iptv(name,url, iconimage):
    fix = "RbDRnK5bYQ/fabiolmg" #coloque usuario premium aqui
    tam = len(url)
    prefix = url[0:32]
    ...
    if tam == 52:
        prefix = url[0:33]
        sufix  = url[tam - 8:tam]
        base_url =""+prefix+""+fix+""+sufix+""
        PlayUrl(name,base_url, iconimage)
